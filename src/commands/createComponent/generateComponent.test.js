...
const compTemplate = require('./componentTemplateUtils');
const { describe, test, beforeEach, afterEach } = require('mocha');
const sinon = require('sinon');
const assert = require('assert');
const message = require('../vscodeHelper/message');
const vsHelper = require('../vscodeHelper/vscodeHelpers');
const fileOperation  = require('../vscodeHelper/fileOperations');

describe('generateComponent', () => {
  
  const  getComponentNameStub = sinon.stub(vsHelper, 'getComponentName');  
  const  fetchWorkspaceFoldersStub = sinon.stub(vsHelper, 'fetchWorkspaceFolders');
  const  processContextMenuPathStub = sinon.stub(vsHelper, 'processContextMenuPath');
  ...
  const  generateComponentFilesStub = sinon.stub(compTemplate, 'generateComponentFiles');

  const  createFilesWithContentStub = sinon.stub(fileOperation, 'createFilesWithContent');
  
  const  showInformationMessageStub = sinon.stub(message, 'showInformationMessage');
